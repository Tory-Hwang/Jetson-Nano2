...
def detect(save_img=False):
    ...
    if webcam:
        ...
        dataset = LoadStreams(source,  h,w, frame_size, img_size=imgsz, stride=stride)
    else:
    ...
    with open('data/coco128_kor.yaml') as f:
        _names = yaml.load(f, Loader=yaml.FullLoader)
    names = _names['names']
    f.close()

    colors = [[random.randint(100, 255) for _ in range(3)] for _ in names]
...
if __name__ == '__main__':
    ...
    if brtsp:
        parser.add_argument('--source', type=str, default='rtsp://admin:satech1234@192.168.0.151:554/udp/av0_0', help='source')  # file/folder, 0 for webcam
