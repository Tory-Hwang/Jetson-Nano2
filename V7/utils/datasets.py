...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', h=0, w=0, frame_size =0, img_size=640, stride=32):
        self.mode = 'stream'
        self.img_size = img_size
        self.stride = stride
        ...
        for i, s in enumerate(sources):
            ...
            if s == '0' or s=='1' :
                ...
                thread = Thread(target=self.update_rtsp, args=([frame_size, i,  h,w, ffmpeg_process, s]), daemon=True)
                print(f' success ({w}x{h} at {self.fps:.2f} FPS).')
                thread.start()
    ...
    def update_rtsp(self, one_frame_size, index, h,w, ffmpeg_process, s):
        # Read stream `i` frames in daemon thread
        #n, f = 0, self.frames[index]
        while True:
