...
def oneframe_size( source ):
    ...
    if is_url :
        ...
        w = probe_dct['streams'][0]['width']
        h = probe_dct['streams'][0]['height']
...
def run(
        ...
        data=ROOT / 'data/coco128_kor.yaml',  # dataset.yaml path
    ...
    if webcam:
        ...
        h, w = oneframe_size(source)
        frame_size = h*w*3

        dataset = LoadStreams(source, h,w, frame_size, img_size=imgsz, stride=stride, auto=pt, vid_stride=vid_stride)
        bs = len(dataset)
    elif screenshot:
...
def parse_opt():
    ...
    if brtsp:
        parser.add_argument('--source', type=str, default='rtsp://admin:satech1234@192.168.0.151:554/udp/av0_0', help='file/dir/URL/glob/screen/0(webcam)')
        parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[1280], help='inference size h,w')
    else:
        parser.add_argument('--source', type=str, default='0', help='file/dir/URL/glob/screen/0(webcam)')
        parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[640], help='inference size h,w')
       
    parser.add_argument('--data', type=str, default=ROOT / 'data/coco128_kor.yaml', help='(optional) dataset.yaml path')   
