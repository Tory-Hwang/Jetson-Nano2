...
from utils.augmentations import (Albumentations, augment_hsv, classify_albumentations, classify_transforms, copy_paste, letterbox, mixup, random_perspective)
from utils.general import (DATASETS_DIR, LOGGER, NUM_THREADS, TQDM_BAR_FORMAT, check_dataset, check_requirements, check_yaml, clean_str, cv2, is_colab, is_kaggle, segments2boxes, unzip_file, xyn2xy, xywh2xyxy, xywhn2xyxy, xyxy2xywhn)
...
class LoadStreams:
    ...
    def __init__(self, sources='file.streams', h=0, w=0, frame_size =0, img_size=640, stride=32, auto=True, transforms=None, vid_stride=1):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            if s == 0 or s==1 :
                ...
                self.imgs[i] = frame.copy() # guarantee first frame
                self.threads[i] = Thread(target=self.update_rtsp, args=([frame_size, i,  h,w, ffmpeg_process, s]), daemon=True)
    ...
    def update_rtsp(self, one_frame_size, i, h,w, ffmpeg_process, s):
        # Read stream `i` frames in daemon thread
        n, f = 0, self.frames[i]
        while True:
